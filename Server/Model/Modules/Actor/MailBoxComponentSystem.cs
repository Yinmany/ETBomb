            ...
            using (await CoroutineLockComponent.Instance.Wait(CoroutineLockType.Mailbox, message.ActorId))
            {
                switch (self.MailboxType)
                {
                    case MailboxType.GateSession:
                        IActorMessage iActorMessage = message;
                        // 发送给客户端
                    ...
                    case MailboxType.MessageDispatcher:
                        await ActorMessageDispatcherComponent.Instance.Handle(self.Parent, session, message, self.IgnoreNotFondHandlerException);
                        break;
